...
(defvar lispy-mode-map-paredit
  (let ((map (copy-keymap lispy-mode-map-base)))
	...
	(define-key map (kbd "M-d") 'paxedit-kill)
    (define-key map (kbd "M-c") 'paxedit-wrap-comment)
    ;(define-key map (kbd "C-o") 'paxedit-compress)
    ;(define-key map (kbd "C-O") 'paxedit-compress)
    ;(define-key map (kbd "C-m") 'paxedit-format-1)
    ;(define-key map (kbd "C-M") 'paxedit-format-1)
