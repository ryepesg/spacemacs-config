(setq ricardo-packages
      '(
        ...
        ;; see https://github.com/syl20bnr/ricardo/issues/2529
...
(if  (version< emacs-version "24.4")
    (push '(paradox :location local) ricardo-packages)
  (push 'paradox ricardo-packages))

;; Initialization of packages

(defun ricardo/init-ace-link ()
  (use-package ace-link
    :commands ricardo/ace-buffer-links
    :init
    (progn
      (define-key spacemacs-mode-map "o" 'ricardo/ace-buffer-links)
    ...
    (progn
      ...
      (defun ricardo//collect-spacemacs-buffer-links ()
        (let ((end (window-end))
              points)
          (save-excursion
            ...
            (nreverse points))))
      (defun ricardo/ace-buffer-links ()
        "Ace jump to links in `spacemacs' buffer."
        (interactive)
        (let ((res (avy--with-avy-keys ricardo/ace-buffer-links
                    (avy--process
                        (ricardo//collect-spacemacs-buffer-links)
                        #'avy--overlay-pre))))
            (when res
            (goto-char (1+ res))
...
(defun ricardo/init-adaptive-wrap ()
  (use-package adaptive-wrap
    :config
    (progn
      (add-hook 'visual-line-mode-hook 'adaptive-wrap-prefix-mode))))

(defun ricardo/init-aggressive-indent ()
  (use-package aggressive-indent
    ...
    :config
    (progn
      (add-hook 'diff-auto-refine-mode-hook 'ricardo/toggle-aggressive-indent-off)
      (spacemacs|diminish aggressive-indent-mode " Ⓘ" " I"))))

(defun ricardo/init-auto-dictionary ()
...
(defun ricardo/init-auto-highlight-symbol ()
  (use-package auto-highlight-symbol
    ...
    (progn
      ...
      (spacemacs|add-toggle automatic-symbol-highlight
        ...
        :evil-leader "tha")
      (ricardo/add-to-hooks 'auto-highlight-symbol-mode '(prog-mode-hook
                                                            markdown-mode-hook)))
    :config
    (progn
      ...
      (defun ricardo/goto-last-searched-ahs-symbol ()
...
`auto-highlight-symbol'."
        ...
        (if spacemacs-last-ahs-highlight-p
            (progn (goto-char (nth 1 spacemacs-last-ahs-highlight-p))
                   (ricardo/ahs-highlight-now-wrapper)
                   (ricardo/symbol-highlight-micro-state))
          (message "No symbol has been searched for now.")))

      (defun ricardo/integrate-evil-search (forward)
      ...
      (defun ricardo/ensure-ahs-enabled-locally ()
      ...
      (defun ricardo/ahs-highlight-now-wrapper ()
        "Safe wrapper for ahs-highlight-now"
        (eval '(progn
                 (ricardo/ensure-ahs-enabled-locally)
                 (ahs-highlight-now)) nil))

      (defun ricardo/enter-ahs-forward ()
        "Go to the next occurrence of symbol under point with
`auto-highlight-symbol'"
        (interactive)
        (setq spacemacs--ahs-searching-forward t)
        (ricardo/quick-ahs-forward))

      (defun ricardo/enter-ahs-backward ()
...
`auto-highlight-symbol'"
      ...
      (defun ricardo/quick-ahs-forward ()
        "Go to the next occurrence of symbol under point with
`auto-highlight-symbol'"
        (interactive)
        (ricardo//quick-ahs-move t))

      (defun ricardo/quick-ahs-backward ()
        "Go to the previous occurrence of symbol under point with
`auto-highlight-symbol'"
        (interactive)
        (ricardo//quick-ahs-move nil))

      (defun ricardo//quick-ahs-move (forward)
        "Go to the next occurrence of symbol under point with
`auto-highlight-symbol'"

        (if (eq forward spacemacs--ahs-searching-forward)
            (progn
              (ricardo/integrate-evil-search t)
              (ricardo/ahs-highlight-now-wrapper)
          ...
          (progn
            (ricardo/integrate-evil-search nil)
            (ricardo/ahs-highlight-now-wrapper)
            (when (configuration-layer/package-usedp 'evil-jumper)
              (evil-set-jump))
            (ricardo/symbol-highlight-micro-state)
      ...
      (eval-after-load 'evil
        '(progn
           (define-key evil-motion-state-map (kbd "*")
             'ricardo/enter-ahs-forward)
           (define-key evil-motion-state-map (kbd "#")
             'ricardo/enter-ahs-backward)))

      (defun ricardo/symbol-highlight ()
      ...
      (defun ricardo/symbol-highlight-reset-range ()
        "Reset the range for `auto-highlight-symbol'."
        (interactive)
        (ahs-change-range ahs-default-range))

      (evil-leader/set-key
        "sh" 'ricardo/symbol-highlight
        "sH" 'ricardo/goto-last-searched-ahs-symbol)
      ...
      (dolist (sym '(ahs-forward
                     ...
                     ahs-back-to-start
                     ahs-change-range))
        (let* ((advice (intern (format "ricardo/%s" (symbol-name sym)))))
      ...
      (spacemacs|define-micro-state symbol-highlight
        ...
        :on-exit  (ricardo//ahs-ms-on-exit)
        :bindings
        ...
        ("e" nil
         :post (if (configuration-layer/package-usedp 'evil-iedit-state)
                 ...
                 (ahs-edit-mode))
         :exit t)
        ("n" ricardo/quick-ahs-forward)
        ("N" ricardo/quick-ahs-backward)
        ("R" ahs-back-to-start)
        ("r" ahs-change-range)
        ("/" ricardo/helm-project-smart-do-search-region-or-symbol :exit t)
        ("b" ricardo/helm-buffers-smart-do-search-region-or-symbol :exit t)
        ("f" ricardo/helm-files-smart-do-search-region-or-symbol :exit t)
        ("q" nil :exit t)))))

(defun ricardo/init-avy ()
  (use-package avy
    :defer t
    :init
    (progn
...
(defun ricardo/init-buffer-move ()
...
(defun ricardo/init-centered-cursor ()
...
(defun ricardo/init-clean-aindent-mode ()
  (use-package clean-aindent-mode
    :defer t
    :init
    (add-hook 'prog-mode-hook 'clean-aindent-mode)))

(defun ricardo/init-desktop ()
  (use-package desktop
    :defer t
    :config
    (progn
...
(defun ricardo/init-define-word ()
...
(defun ricardo/init-dired+ ()
  (use-package dired+
    :defer t))

(defun ricardo/init-doc-view ()
  (use-package doc-view
    :defer t
    :init
    (evilify doc-view-mode doc-view-mode-map
             "/"  'ricardo/doc-view-search-new-query
             "?"  'ricardo/doc-view-search-new-query-backward
    ...
    :config
    (progn
      (defun ricardo/doc-view-search-new-query ()
        "Initiate a new query."
        (interactive)
        (doc-view-search 'newquery))

      (defun ricardo/doc-view-search-new-query-backward ()
        "Initiate a new query."
        (interactive)
      ...
      (defadvice doc-view-toggle-display
          (around ricardo/doc-view-toggle-display activate)
...
(defun ricardo/init-eval-sexp-fu ()
  ...
  (use-package evil-anzu
    ...
    (progn
      ...
      (when (configuration-layer/package-usedp 'powerline)
        (defun ricardo/anzu-update-mode-line (here total)
          ...
          (when anzu--state
            (let ((status (cl-case anzu--state
              ...
              status)))
        (setq anzu-mode-line-update-function 'ricardo/anzu-update-mode-line)))))

(defun ricardo/init-evil-args ()
...
(defun ricardo/init-evil-exchange ()
  (use-package evil-exchange
    :init (evil-exchange-install)))

(defun ricardo/init-evil-iedit-state ()
...
(defun ricardo/init-evil-indent-textobject ()
  (use-package evil-indent-textobject))

(defun ricardo/init-evil-jumper ()
...
(defun ricardo/init-evil-lisp-state ()
...
(defun ricardo/init-evil-nerd-commenter ()
  (use-package evil-nerd-commenter
    ...
    (progn
      ...
      (defun ricardo/comment-or-uncomment-lines-inverse (&optional arg)
        (interactive "p")
        (let ((evilnc-invert-comment-line-by-line t))
          (evilnc-comment-or-uncomment-lines arg)))

      (defun ricardo/comment-or-uncomment-lines (&optional arg)
        (interactive "p")
        (let ((evilnc-invert-comment-line-by-line nil))
          (evilnc-comment-or-uncomment-lines arg)))

      (defun ricardo/copy-and-comment-lines-inverse (&optional arg)
        (interactive "p")
        (let ((evilnc-invert-comment-line-by-line t))
          (evilnc-copy-and-comment-lines arg)))

      (defun ricardo/copy-and-comment-lines (&optional arg)
        (interactive "p")
        (let ((evilnc-invert-comment-line-by-line nil))
          (evilnc-copy-and-comment-lines arg)))

      (defun ricardo/quick-comment-or-uncomment-to-the-line-inverse
          (&optional arg)
        (interactive "p")
        (let ((evilnc-invert-comment-line-by-line t))
          (evilnc-comment-or-uncomment-to-the-line arg)))

      (defun ricardo/quick-comment-or-uncomment-to-the-line (&optional arg)
        (interactive "p")
        (let ((evilnc-invert-comment-line-by-line nil))
          (evilnc-comment-or-uncomment-to-the-line arg)))

      (defun ricardo/comment-or-uncomment-paragraphs-inverse (&optional arg)
        (interactive "p")
        (let ((evilnc-invert-comment-line-by-line t))
          (evilnc-comment-or-uncomment-paragraphs arg)))

      (defun ricardo/comment-or-uncomment-paragraphs (&optional arg)
        (interactive "p")
        (let ((evilnc-invert-comment-line-by-line nil))
          (evilnc-comment-or-uncomment-paragraphs arg)))

      (define-key evil-normal-state-map "gc" 'evilnc-comment-operator)
      (define-key evil-normal-state-map "gy" 'ricardo/copy-and-comment-lines)

      (evil-leader/set-key
        ";"  'evilnc-comment-operator
        "cl" 'ricardo/comment-or-uncomment-lines
        "cL" 'ricardo/comment-or-uncomment-lines-inverse
        "cp" 'ricardo/comment-or-uncomment-paragraphs
        "cP" 'ricardo/comment-or-uncomment-paragraphs-inverse
        "ct" 'ricardo/quick-comment-or-uncomment-to-the-line
        "cT" 'ricardo/quick-comment-or-uncomment-to-the-line-inverse
        "cy" 'ricardo/copy-and-comment-lines
        "cY" 'ricardo/copy-and-comment-lines-inverse))))

(defun ricardo/init-evil-matchit ()
  (use-package evil-matchit
    :defer t))

(defun ricardo/init-evil-numbers ()
  (use-package evil-numbers
    :config
    (progn
      (defun ricardo/evil-numbers-micro-state-doc ()
        "Display a short documentation in the mini buffer."
        (ricardo/echo "+/= to increase the value or - to decrease it"))

      (defun ricardo/evil-numbers-micro-state-overlay-map ()
        "Set a temporary overlay map to easily increase or decrease a number"
        (set-temporary-overlay-map
         (let ((map (make-sparse-keymap)))
           (define-key map (kbd "+") 'ricardo/evil-numbers-increase)
           (define-key map (kbd "=") 'ricardo/evil-numbers-increase)
           (define-key map (kbd "-") 'ricardo/evil-numbers-decrease)
           map) t)
        (ricardo/evil-numbers-micro-state-doc))

      (defun ricardo/evil-numbers-increase (amount &optional no-region)
        "Increase number at point."
        (interactive "p*")
        (evil-numbers/inc-at-pt amount no-region)
        (ricardo/evil-numbers-micro-state-overlay-map))
      (defun ricardo/evil-numbers-decrease (amount)
        "Decrease number at point."
        (interactive "p*")
        (evil-numbers/dec-at-pt amount)
        (ricardo/evil-numbers-micro-state-overlay-map))
      (evil-leader/set-key "n+" 'ricardo/evil-numbers-increase)
      (evil-leader/set-key "n=" 'ricardo/evil-numbers-increase)
      (evil-leader/set-key "n-" 'ricardo/evil-numbers-decrease))))

(defun ricardo/init-evil-search-highlight-persist ()
  (use-package evil-search-highlight-persist
    ...
    (progn
      ...
      (defun ricardo/adaptive-evil-highlight-persist-face ()
        (set-face-attribute 'evil-search-highlight-persist-highlight-face nil
                            :inherit 'region
                            :background nil
                            :foreground nil))
      (ricardo/adaptive-evil-highlight-persist-face))))

(defun ricardo/init-evil-terminal-cursor-changer ()
...
(defun ricardo/init-expand-region ()
        ...
        (defadvice er/prepare-for-more-expansions-internal
          ...
          (let ((new-msg (concat (car ad-return-value)
            ...
            (cl-pushnew
             '("/" (lambda ()
                     (call-interactively
                      'ricardo/helm-project-smart-do-search-region-or-symbol)))
             new-bindings)
            (cl-pushnew
             '("f" (lambda ()
                     (call-interactively
                      'ricardo/helm-files-smart-do-search-region-or-symbol)))
             new-bindings)
            (cl-pushnew
             '("b" (lambda ()
                     (call-interactively
                      'ricardo/helm-buffers-smart-do-search-region-or-symbol)))
             new-bindings)
            (setq ad-return-value (cons new-msg new-bindings)))))
      (setq expand-region-contract-fast-key "V"
            expand-region-reset-fast-key "r"))))

(defun ricardo/init-fancy-battery ()
  (use-package fancy-battery
    ...
    (progn
      ...
      (defun ricardo/mode-line-battery-percentage ()
      ...
      (defun ricardo/mode-line-battery-time ()
      ...
      (defun fancy-battery-default-mode-line ()
        ...
        (when fancy-battery-last-status
          (let* ((type (cdr (assq ?L fancy-battery-last-status)))
                 (percentage (ricardo/mode-line-battery-percentage))
                 (time (ricardo/mode-line-battery-time)))
            (cond
...
(defun ricardo/init-flx-ido ()
  (use-package flx-ido
    :init (flx-ido-mode 1)))

(defun ricardo/init-golden-ratio ()
  (use-package golden-ratio
    ...
    (progn
      ...
      (defun ricardo/no-golden-ratio-for-buffers (bufname)
        "Disable golden-ratio if BUFNAME is the name of a visible buffer."
        (and (get-buffer bufname) (get-buffer-window bufname 'visible)))
      (defun ricardo/no-golden-ratio-guide-key ()
        "Disable golden-ratio for guide-key popwin buffer."
        (or (ricardo/no-golden-ratio-for-buffers " *guide-key*")
            (ricardo/no-golden-ratio-for-buffers " *popwin-dummy*")))
      (add-to-list 'golden-ratio-inhibit-functions
                   'ricardo/no-golden-ratio-guide-key)
...
(defun ricardo/init-google-translate ()
  (use-package google-translate
    ...
    :init
    (progn
      (defun ricardo/set-google-translate-languages (source target)
...
(defun ricardo/init-helm-ag ()
  (use-package helm-ag
    :defer t
    :init
    (progn
      (defun ricardo//helm-do-ag-region-or-symbol (func &optional dir)
      ...
      (defun ricardo//helm-do-search-find-tool (base tools default-inputp)
      ...
      (defun ricardo/helm-file-do-ag (&optional _)
        "Wrapper to execute `helm-ag-this-file.'"
        (interactive)
        (helm-ag-this-file))

      (defun ricardo/helm-file-do-ag-region-or-symbol ()
        "Search in current file with `ag' using a default input."
        (interactive)
        (ricardo//helm-do-ag-region-or-symbol 'ricardo/helm-file-do-ag))

      (defun ricardo/helm-file-smart-do-search (&optional default-inputp)
...
are used as default input."
        (interactive)
        (call-interactively
         (ricardo//helm-do-search-find-tool "helm-file-do"
                                              dotspacemacs-search-tools
                                              default-inputp)))

      (defun ricardo/helm-file-smart-do-search-region-or-symbol ()
        "Search in current file using `dotspacemacs-search-tools' with
 default input.
Search for a search tool in the order provided by `dotspacemacs-search-tools'."
        (interactive)
        (ricardo/helm-file-smart-do-search t))

      ;; Search in files -----------------------------------------------------

      (defun ricardo/helm-files-do-ag (&optional dir)
        "Search in files with `ag' using a default input."
        (interactive)
        (helm-do-ag dir))

      (defun ricardo/helm-files-do-ag-region-or-symbol ()
        "Search in files with `ag' using a default input."
        (interactive)
        (ricardo//helm-do-ag-region-or-symbol 'ricardo/helm-files-do-ag))

      (defun ricardo/helm-files-do-ack (&optional dir)
        "Search in files with `ack'."
        (interactive)
        (let ((helm-ag-base-command "ack --nocolor --nogroup"))
          (helm-do-ag dir)))

      (defun ricardo/helm-files-do-ack-region-or-symbol ()
        "Search in files with `ack' using a default input."
        (interactive)
        (ricardo//helm-do-ag-region-or-symbol 'ricardo/helm-files-do-ack))

      (defun ricardo/helm-files-do-pt (&optional dir)
        "Search in files with `pt'."
        (interactive)
        (let ((helm-ag-base-command "pt -e --nocolor --nogroup"))
          (helm-do-ag dir)))

      (defun ricardo/helm-files-do-pt-region-or-symbol ()
        "Search in files with `pt' using a default input."
        (interactive)
        (ricardo//helm-do-ag-region-or-symbol 'ricardo/helm-files-do-pt))

      (defun ricardo/helm-files-smart-do-search (&optional default-inputp)
...
are used as default input."
        (interactive)
        (call-interactively
         (ricardo//helm-do-search-find-tool "helm-files-do"
                                              dotspacemacs-search-tools
                                              default-inputp)))

      (defun ricardo/helm-files-smart-do-search-region-or-symbol ()
        "Search in opened buffers using `dotspacemacs-search-tools'.
with default input.
Search for a search tool in the order provided by `dotspacemacs-search-tools'."
        (interactive)
        (ricardo/helm-files-smart-do-search t))

      ;; Search in buffers ---------------------------------------------------

      (defun ricardo/helm-buffers-do-ag (&optional _)
        "Wrapper to execute `helm-ag-buffers.'"
        (interactive)
        (helm-do-ag-buffers))

      (defun ricardo/helm-buffers-do-ag-region-or-symbol ()
        "Search in opened buffers with `ag' with a default input."
        (interactive)
        (ricardo//helm-do-ag-region-or-symbol 'ricardo/helm-buffers-do-ag))

      (defun ricardo/helm-buffers-do-ack (&optional _)
        "Search in opened buffers with `ack'."
        (interactive)
        (let ((helm-ag-base-command "ack --nocolor --nogroup"))
          (helm-do-ag-buffers)))

      (defun ricardo/helm-buffers-do-ack-region-or-symbol ()
        "Search in opened buffers with `ack' with a default input."
        (interactive)
        (ricardo//helm-do-ag-region-or-symbol 'ricardo/helm-buffers-do-ack))

      (defun ricardo/helm-buffers-do-pt (&optional _)
        "Search in opened buffers with `pt'."
        (interactive)
        (let ((helm-ag-base-command "pt -e --nocolor --nogroup"))
          (helm-do-ag-buffers)))

      (defun ricardo/helm-buffers-do-pt-region-or-symbol ()
        "Search in opened buffers with `pt' using a default input."
        (interactive)
        (ricardo//helm-do-ag-region-or-symbol 'ricardo/helm-buffers-do-pt))

      (defun ricardo/helm-buffers-smart-do-search (&optional default-inputp)
        "Search in opened buffers using `dotspacemacs-search-tools'.
Search for a search tool in the order provided by `dotspacemacs-search-tools'
If DEFAULT-INPUTP is non nil then the current region or symbol at point
are used as default input."
        (interactive)
        (call-interactively
         (ricardo//helm-do-search-find-tool "helm-buffers-do"
                                              dotspacemacs-search-tools
                                              default-inputp)))

      (defun ricardo/helm-buffers-smart-do-search-region-or-symbol ()
        "Search in opened buffers using `dotspacemacs-search-tools' with
default input.
Search for a search tool in the order provided by `dotspacemacs-search-tools'."
        (interactive)
        (ricardo/helm-buffers-smart-do-search t))

      ;; Search in project ---------------------------------------------------

      (defun ricardo/helm-project-do-ag ()
      ...
      (defun ricardo/helm-project-do-ag-region-or-symbol ()
        "Search in current project with `ag' using a default input."
        (interactive)
        (let ((dir (projectile-project-root)))
          (if dir
              (ricardo//helm-do-ag-region-or-symbol 'helm-do-ag dir)
            (message "error: Not in a project."))))

      (defun ricardo/helm-project-do-ack ()
        "Search in current project with `ack'."
        (interactive)
        (let ((dir (projectile-project-root)))
          (if dir
              (ricardo/helm-files-do-ack dir)
            (message "error: Not in a project."))))

      (defun ricardo/helm-project-do-ack-region-or-symbol ()
        "Search in current project with `ack' using a default input."
        (interactive)
        (let ((dir (projectile-project-root)))
          (if dir
              (ricardo//helm-do-ag-region-or-symbol 'ricardo/helm-files-do-ack dir)
            (message "error: Not in a project."))))

      (defun ricardo/helm-project-do-pt ()
        "Search in current project with `pt'."
        (interactive)
        (let ((dir (projectile-project-root)))
          (if dir
              (ricardo/helm-files-do-pt dir)
            (message "error: Not in a project."))))

      (defun ricardo/helm-project-do-pt-region-or-symbol ()
        "Search in current project with `pt' using a default input."
        (interactive)
        (let ((dir (projectile-project-root)))
          (if dir
              (ricardo//helm-do-ag-region-or-symbol 'ricardo/helm-files-do-pt dir)
            (message "error: Not in a project."))))

      (defun ricardo/helm-project-smart-do-search (&optional default-inputp)
...
are used as default input."
        ...
        (let ((projectile-require-project-root nil))
         (call-interactively
          (ricardo//helm-do-search-find-tool "helm-project-do"
                                               dotspacemacs-search-tools
                                               default-inputp))))

      (defun ricardo/helm-project-smart-do-search-region-or-symbol ()
        "Search in current project using `dotspacemacs-search-tools' with
 default input.
Search for a search tool in the order provided by `dotspacemacs-search-tools'."
        (interactive)
        (ricardo/helm-project-smart-do-search t))
      ...
      (evil-leader/set-key
        ...
        "sb"  'ricardo/helm-buffers-smart-do-search
        "sB"  'ricardo/helm-buffers-smart-do-search-region-or-symbol
        "sab" 'helm-do-ag-buffers
        "saB" 'ricardo/helm-buffers-do-ag-region-or-symbol
        "skb" 'ricardo/helm-buffers-do-ack
        "skB" 'ricardo/helm-buffers-do-ack-region-or-symbol
        "stb" 'ricardo/helm-buffers-do-pt
        "stB" 'ricardo/helm-buffers-do-pt-region-or-symbol
        ;; current file scope
        "ss"  'ricardo/helm-file-smart-do-search
        "sS"  'ricardo/helm-file-smart-do-search-region-or-symbol
        "saa" 'helm-ag-this-file
        "saA" 'ricardo/helm-file-do-ag-region-or-symbol
        ;; files scope
        "sf"  'ricardo/helm-files-smart-do-search
        "sF"  'ricardo/helm-files-smart-do-search-region-or-symbol
        "saf" 'helm-do-ag
        "saF" 'ricardo/helm-files-do-ag-region-or-symbol
        "skf" 'ricardo/helm-files-do-ack
        "skF" 'ricardo/helm-files-do-ack-region-or-symbol
        "stf" 'ricardo/helm-files-do-pt
        "stF" 'ricardo/helm-files-do-pt-region-or-symbol
        ;; current project scope
        "/"   'ricardo/helm-project-smart-do-search-region-or-symbol
        "sp"  'ricardo/helm-project-smart-do-search
        "sP"  'ricardo/helm-project-smart-do-search-region-or-symbol
        "sap" 'ricardo/helm-project-do-ag
        "saP" 'ricardo/helm-project-do-ag-region-or-symbol
        "skp" 'ricardo/helm-project-do-ack
        "skP" 'ricardo/helm-project-do-ack-region-or-symbol
        "stp" 'ricardo/helm-project-do-pt
        "stP" 'ricardo/helm-project-do-pt-region-or-symbol))
    :config
    (progn
...
(defun ricardo/init-helm-mode-manager ()
...
(defun ricardo/init-helm-swoop ()
  (use-package helm-swoop
    ...
    (progn
      ...
      (defun ricardo/helm-swoop-region-or-symbol ()
      ...
      (evil-leader/set-key
        "ss"    'helm-swoop
        "sS"    'ricardo/helm-swoop-region-or-symbol
        "s C-s" 'helm-multi-swoop-all)
      (defadvice helm-swoop (before add-evil-jump activate)
        (when (configuration-layer/package-usedp 'evil-jumper)
          (evil-set-jump))))))

(defun ricardo/init-helm-themes ()
...
(defun ricardo/init-highlight-indentation ()
...
(defun ricardo/init-highlight-numbers ()
...
(defun ricardo/init-highlight-parentheses ()
...
(defun ricardo/init-hl-anything ()
...
(defun ricardo/init-hungry-delete ()
...
(defun ricardo/init-indent-guide ()
...
(defun ricardo/init-open-junk-file ()
...
(defun ricardo/init-leuven-theme ()
  (use-package leuven-theme
    :defer t
    :init (setq org-fontify-whole-heading-line t)))

(defun ricardo/init-linum-relative ()
...
(defun ricardo/init-neotree ()
  (use-package neotree
    ...
    (progn
      ...
      (defun ricardo/neotree-expand-or-open ()
      ...
      (defun ricardo/neotree-collapse ()
        "Collapse a neotree node."
        (interactive)
        (let ((node (neo-buffer--get-filename-current-line)))
          (when node
      ...
      (defun ricardo/neotree-collapse-or-up ()
      ...
      (defun ricardo//neotree-key-bindings ()
        ...
        (define-key evil-motion-state-local-map (kbd "h")   'ricardo/neotree-collapse-or-up)
        ...
        (define-key evil-motion-state-local-map (kbd "l")   'ricardo/neotree-expand-or-open)
    ...
    :config
    (ricardo/add-to-hook 'neotree-mode-hook '(ricardo//neotree-key-bindings))))

(defun ricardo/init-pcre2el ()
  (use-package pcre2el
    :defer t
    :commands rxt-fontify-regexp-at-point
    :init
    (progn
      (ricardo/declare-prefix "R" "pcre2el")
      (evil-leader/set-key
        "R/"  'rxt-explain
...
(defun ricardo/init-paradox ()
  (use-package paradox
    :commands paradox-list-packages
    :init
    (progn
      (setq paradox-execute-asynchronously nil)
      (defun ricardo/paradox-list-packages ()
      ...
      (evil-leader/set-key
        "aP" 'ricardo/paradox-list-packages))))

(defun ricardo/init-powerline ()
                      ...
                      (define-key map
                        ...
                        (lambda (event)
                          ...
                          (if (yes-or-no-p
                              ...
                              (progn
                                (ricardo/switch-to-version spacemacs-new-version)
...
`ricardo//eval-mode-line-segment'.")
...
`ricardo//eval-mode-line-segment'.")
      ...
      (defun ricardo//mode-line-file-encoding ()
      ...
      (defun ricardo/customize-powerline-faces ()
        "Alter powerline face to make them work with more themes."
        (set-face-attribute 'powerline-inactive2 nil
                            :inherit 'font-lock-comment-face))
      (ricardo/customize-powerline-faces)
...
This macro defines a function `ricardo//mode-line-NAME' which
...
inspected at evaluation time by `ricardo//eval-mode-line-segment'."
        (declare (indent 1))
        (let* ((wrapper-func (intern (format "ricardo//mode-line-%S" name)))
...
- or rowsxcols if it's a block selection."
      ...
      (spacemacs|define-mode-line-segment process
        ...
        :when (ricardo//mode-line-nonempty mode-line-process))
      ...
      (spacemacs|define-mode-line-segment buffer-encoding-abbrev
        (ricardo//mode-line-file-encoding))
      ...
      (spacemacs|define-mode-line-segment global-mode
        ...
        :when (ricardo//mode-line-nonempty global-mode-string))
      ...
      (spacemacs|define-mode-line-segment new-version
        (spacemacs-powerline-new-version
         (ricardo/get-new-version-lighter-face
      ...
      (defun ricardo//eval-mode-line-segment (segment-spec &rest outer-props)
...
The return vaule is a `segment' struct. Its `OBJECTS' list may be nil."
        ...
        (let* ((input (if (and (listp segment-spec)
               ...
               (segment-symbol (when (symbolp segment)
                                 (intern (format "ricardo//mode-line-%S" segment))))
            ...
            (cond
             ...
             ((listp segment)
              (let ((results (remove-if-not
                              'segment-objects
                              (mapcar (lambda (s)
                                        (apply 'ricardo//eval-mode-line-segment
                                               s nest-props))
                                      segment))))
                (when results
                  (setf (segment-objects result)
                        (apply 'append (ricardo//intersperse
             ...
             ((symbolp segment)
              (setf (segment-objects result)
                    (mapcar (lambda (s)
                              (if (ricardo//imagep s) s (powerline-raw s face)))
      ...
      (defun ricardo//mode-line-prepare-any (spec side)
        "Prepares one side of the modeline. `SPEC' is a list of segment
specifications (see `ricardo//eval-mode-line-segment'), and `SIDE' is
one of `l' or `r'."
        (let* ((active (powerline-selected-window-active))
               ...
               (state-face (if active (ricardo/current-state-face) line-face))

               ;; Loop through the segments and collect the results
               (segments (loop with result
                               for s in spec
                               do (setq result (ricardo//eval-mode-line-segment s))
      ...
      (defun ricardo//mode-line-prepare-left ()
        (ricardo//mode-line-prepare-any spacemacs-mode-line-left 'l))

      (defun ricardo//mode-line-prepare-right ()
        (ricardo//mode-line-prepare-any spacemacs-mode-line-right 'r))

      (defun ricardo//mode-line-prepare ()
              ...
              (let ((mode (car mm)))
                (when (and (boundp mode) (symbol-value mode))
                  (let* ((unicode (cadr mm))
                    ...
                    (diminish mode dim)))))))
        (let* ((active (powerline-selected-window-active))
               (lhs (ricardo//mode-line-prepare-left))
               (rhs (ricardo//mode-line-prepare-right))
               (line-face (if active 'powerline-active2 'powerline-inactive2)))
          ;; create the line
      ...
      (setq-default mode-line-format
                    '("%e" (:eval (ricardo//mode-line-prepare))))

      (defun ricardo//restore-powerline (buffer)
      ...
      (defun ricardo//set-powerline-for-startup-buffers ()
        "Set the powerline for buffers created when Emacs starts."
        (unless configuration-layer-error-count
          (dolist (buffer '("*Messages*" "*spacemacs*" "*Compile-Log*"))
            (when (and (get-buffer buffer)
                       (configuration-layer/package-usedp 'powerline))
              (ricardo//restore-powerline buffer)))))
      (add-hook 'emacs-startup-hook
                'ricardo//set-powerline-for-startup-buffers))))

(defun ricardo/init-rainbow-delimiters ()
  (use-package rainbow-delimiters
    ...
    (progn
      ...
      (when (member dotspacemacs-highlight-delimiters '(any all))
        (ricardo/add-to-hooks 'rainbow-delimiters-mode '(prog-mode-hook))))))

(defun ricardo/init-smartparens ()
  (use-package smartparens
    :defer t
    :init
    (progn
      (ricardo/add-to-hooks (if dotspacemacs-smartparens-strict-mode
    ...
    (progn
      ...
      (defun ricardo/smartparens-pair-newline (id action context)
        (save-excursion
          (newline)
          (indent-according-to-mode)))

      (defun ricardo/smartparens-pair-newline-and-indent (id action context)
        (ricardo/smartparens-pair-newline id action context)
        (indent-according-to-mode))
      ...
      (sp-pair "{" nil :post-handlers
               '(:add (ricardo/smartparens-pair-newline-and-indent "RET")))
      (sp-pair "[" nil :post-handlers
               '(:add (ricardo/smartparens-pair-newline-and-indent "RET"))))))

(defun ricardo/init-smooth-scrolling ()
...
(defun ricardo/init-solarized-theme ()
...
(defun ricardo/init-spray ()
  (use-package spray
    :commands spray-mode
    :init
    (progn
      (defun ricardo/start-spray ()
        "Start spray speed reading on current buffer at current point."
        (interactive)
        (evil-insert-state)
        (spray-mode t)
        (internal-show-cursor (selected-window) nil))
      (evil-leader/set-key "asr" 'ricardo/start-spray)

      (defadvice spray-quit (after ricardo//quit-spray activate)
...
(defun ricardo/init-vi-tilde-fringe ()
...
(defun ricardo/init-window-numbering ()
  (use-package window-numbering
    ...
    (defun ricardo//window-numbering-assign (windows)
      ...
      (mapc (lambda (w)
            ...
            windows))
    (add-hook 'window-numbering-before-hook 'ricardo//window-numbering-assign)
    (add-hook 'neo-after-create-hook '(lambda (w) (window-numbering-update)))))

(defun ricardo/init-volatile-highlights ()
  ...
  (use-package zoom-frm
    ...
    (progn
      (spacemacs|define-micro-state zoom-frm
        ...
        :use-minibuffer t
        :bindings
        ("+" ricardo/zoom-frm-in :post (ricardo//zoom-frm-powerline-reset))
        ("-" ricardo/zoom-frm-out :post (ricardo//zoom-frm-powerline-reset))
        ("=" ricardo/zoom-frm-unzoom :post (ricardo//zoom-frm-powerline-reset)))

      (defun ricardo//zoom-frm-powerline-reset ()
        (when (fboundp 'powerline-reset)
          (setq-default powerline-height (ricardo/compute-powerline-height))
          (powerline-reset)))

      (defun ricardo//zoom-frm-do (arg)
      ...
      (defun ricardo/zoom-frm-in ()
        "zoom in frame, but keep the same pixel size"
        (interactive)
        (ricardo//zoom-frm-do 1))

      (defun ricardo/zoom-frm-out ()
        "zoom out frame, but keep the same pixel size"
        (interactive)
        (ricardo//zoom-frm-do -1))

      (defun ricardo/zoom-frm-unzoom ()
        "Unzoom current frame, keeping the same pixel size"
        (interactive)
        (ricardo//zoom-frm-do 0))

      ;; Font size, either with ctrl + mouse wheel
      (global-set-key (kbd "<C-wheel-up>") 'ricardo/zoom-frm-in)
      (global-set-key (kbd "<C-wheel-down>") 'ricardo/zoom-frm-out))))
